...
export class ExecutionCircleController {
  ...
  getFeed = async (
    ...
    try {
      ...
      if (!circleId) {
        const circle = await this.service.getUserCircle(userId);
        if (!circle) {
          res.status(404).json({ error: "Join a circle first" });
          return;
        }
        circleId = circle.id;
      }
  ...
  completeChallenge = async (
    ...
    try {
      ...
      if (!challenge_id) {
        res.status(400).json({ error: "challenge_id required" });
        return;
      }
  ...
  uploadProof = async (
    ...
    try {
      ...
      // Validate UUID
      UUIDParamSchema.parse({ circleId: circle_id });
  ...
  requestChange = async (
    ...
    try {
      ...
      const { circle_id, reason } = req.body as {
        circle_id: string;
        reason: string;
      };
      ...
      if (!circle_id || !reason) {
        res.status(400).json({
          error: "circle_id and reason are required",
        });
        return;
      }
      ...
      const result = await this.service.requestCircleChange(
        userId,
        circle_id,
        reason,
      );
