...
export class RoadmapService {
  ...
  static async generateRoadmap(userId: string, input: any) {
    ...
    for (const phase of parsed.phases) {
      const totalMinutes = phase.tasks.reduce((acc, task) => acc + task.estimated_minutes, 0);
      const maxMinutes = phase.duration_weeks * 7 * (input.dailyStudyMinutes || 120);
      if (totalMinutes > maxMinutes * 1.2) {
        throw new Error(`Unrealistic workload in phase: ${phase.title}`);
      }
    }
    ...
    // 8 - GENERATE FIRST DAILY TASK
    await dailyTaskService.generateDailyTasks(userId);

    // =========================================================
    // 9 - AUTO MATCH EXECUTION CIRCLE (THE MISSING LINK)
    // =========================================================
    try {
      const circleService = new ExecutionCircleService(supabase);
      
      // Parse the duration string from onboarding into a number
